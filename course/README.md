...
- **授權**：本 repo 採雙授權（2026-09-11 起）——程式碼 MIT（`LICENSE`）、
  **`course/` 全部教材 CC BY-NC-SA 4.0**（`LICENSE-CONTENT`）：要標名字、**不可商業使用**、
  改作要用同樣條款開放。重用教材時請一併保留各臨床來源的出處標示；
  官方基準表與 EM Model 本身不在本授權範圍內。
