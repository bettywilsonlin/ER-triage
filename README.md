...
雙授權，**界線是「拿掉之後還跑不跑得動」**——拿掉不影響程式運行的，算內容。

| | 條款 | 白話 |
|---|---|---|
| **程式碼**（`src/` 的邏輯、`public/`、`test/`、設定檔）| MIT（[`LICENSE`](LICENSE)）| 隨便拿去改、拿去用，保留版權聲明就好 |
| **教材與題目**（`course/` 全部、各 `.md`、題目文字與計分規則）| CC BY-NC-SA 4.0（[`LICENSE-CONTENT`](LICENSE-CONTENT)）| 要標名字、**不可商業使用**、改作要用同樣條款開放 |

> ⚠️ `src/questions.js` 與 `src/content_v2.js` 是**混合檔**：程式結構走 MIT，裡面承載的題目與教材文字走 CC BY-NC-SA。**想借架構、自己換一套題目 → 不受 NC 限制**；想沿用題目本身 → 受限。

院內教學、PGY 訓練、學會非營利工作坊都在 NC 允許範圍內；**收費課程或營利機構的付費訓練不行**，要另外談書面授權。

檢傷題目是**依「急診五級檢傷分類基準」自行撰寫的教學題目**——授權只及於這些題目本身，基準表與 EM Model 的權利屬原權利人，不在範圍內。

> ⚠️ 這是**教學模擬工具，不是臨床決策輔助系統**。所有數值與案例都是教學設計，不得用於實際病人的檢傷判斷。完整免責聲明見 [`LICENSE-CONTENT`](LICENSE-CONTENT) 末節。
